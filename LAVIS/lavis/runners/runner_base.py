...
class RunnerBase:
 ...
 ## Commented before setting parameter to false
    ...
    def optimizer(self):
        ...
        if self._optimizer is None:
            ...
            beta2 = self.config.run_cfg.get("beta2", 0.98)

            if self.config.run_cfg.get("use_adam", False):
    ...
    def train(self):
        ...
        for cur_epoch in range(self.start_epoch, self.max_epoch):
            ...
            if not self.evaluate_only:
                ...
                wandb.log({
                    f"epoch_{k}": float(v) for k,v in train_stats.items()
                })
            ...
            if len(self.valid_splits) > 0:
                for split_name in self.valid_splits:
                    ...
                    if val_log is not None:
                        ...
                        if is_main_process():
                            ...
                            wandb.log({
                                f"{split_name}_{k}": float(v) for k,v in val_log.items()
                            })
                            ...
                            print(agg_metrics)
                            print("==================================")
                            print(best_agg_metric)
                            print('=====================================')

                            if agg_metrics > best_agg_metric and split_name == "val":
                                best_epoch, best_agg_metric = cur_epoch, agg_metrics

                                print('Inside training loop :  Saving Checkpoint')
                ...
                if not self.evaluate_only:
                    print('Inside training loop(else) :  Saving Checkpoint')
                    self._save_checkpoint(cur_epoch, is_best=False)
        ...
        # testing phase
        test_epoch = "best" if len(self.test_splits) > 0 else cur_epoch
    ...
    def unwrap_dist_model(self, model):
        #==========================Changed for unused parameteters======================
        if self.use_distributed:
            return model.module
        else:
            return model
