...
#============================================================================
        ...
        # image_feat_domain_final = self.image_norm(image_feat_old + txt_proj.view(-1,self.num_patches,self.domain_dim))

        # image_feat_domain_final = self.image_norm(image_feat_domain + image_feat_old)
        # image_feats = F.normalize(image_feat_domain_final, dim=-1)

        # #Add the domain cross-attended text-features to the text features
        # # text_feat_domain_final = self.text_norm(text_feat_old + img_proj.squeeze(dim=-1))
        ...
        loss_itm = torch.tensor(0.00, requires_grad=True).to(loss_itc.device)
        loss_itm = loss_itm+loss_itm

        loss_lm = torch.tensor(0.00, requires_grad=True).to(loss_itc.device)
