...
def train(model, data_loader, optimizer, epoch, device, config, args):
    ...
    metric_logger.add_meter('loss_jaccard', utils.SmoothedValue(window_size=1, fmt='{value:.4f}'))
    ...
    for i,metadata in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        loss_ita, loss_jaccard = model(image, caption, idx=idx)         
        loss = 0*loss_ita + loss_jaccard
        ...
        metric_logger.update(loss_jaccard=loss_jaccard.item())
        metric_logger.update(lr=optimizer.param_groups[0]["lr"])
        

        wandb_stats = {
            "global_loss": loss_ita.item(),
            "jaccard_loss": loss_jaccard.item()
            }
...
def main(args, config):
    ...
    from models.blip_jaccard_shared_codebook import blip_retrieval
