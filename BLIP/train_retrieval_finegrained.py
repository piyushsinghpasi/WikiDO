...
def train(model, data_loader, optimizer, epoch, device, config, args):
    ...
    metric_logger.add_meter('loss_token_ita', utils.SmoothedValue(window_size=1, fmt='{value:.4f}'))
    ...
    for i,metadata in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        loss_ita, loss_token_ita = model(image, caption, idx=idx)         
        loss = 0.2*loss_ita + 0.8*loss_token_ita
        ...
        metric_logger.update(loss_token_ita=loss_token_ita.item())
        metric_logger.update(lr=optimizer.param_groups[0]["lr"])
        

        wandb_stats = {
            "global_loss": loss_ita.item(),
            "local_loss": loss_token_ita.item()
            }
...
def evaluation(model, data_loader, device, config, args):
    ...
    for idx, (image, img_id, img_path, caption, topic) in enumerate(data_loader): 
        ...
        #text_embed = torch.mean(text_output.last_hidden_state[:,1:,:], dim=1)
        
        # l_token_embed = model.attention(text_output.last_hidden_state[:,1:,:], text_output.last_hidden_state[:,0,:], text_output.last_hidden_state[:,0,:])
        # text_embed = torch.mean(l_token_embed, dim=1)
        text_embed = model.text_proj(text_output.last_hidden_state[:,0,:])
        image = image.to(device) 
        image_feat = model.visual_encoder(image)   
        #v_patch_embed = model.attention(image_feat[:,1:,:], image_feat[:,0,:], image_feat[:,0,:])
        #image_embed = torch.mean((image_feat[:,1:,:]), dim=1)    
        #image_embed = torch.mean(v_patch_embed, dim=1)    
...
def main(args, config):
    ...
    from models.blip_A import blip_retrieval
