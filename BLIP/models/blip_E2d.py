...
class BLIP_Retrieval(nn.Module):
    def __init__(self,                 
        ...
        self.register_buffer("image_queue_a", torch.randn(embed_dim, queue_size))
        self.register_buffer("text_queue_a", torch.randn(embed_dim, queue_size))
        ...
        self.temp_a = nn.Parameter(0.07*torch.ones([])) 
    ...
    def forward(self, image, caption, idx):
        ...
        v_embed = F.normalize(v_embed_u + v_embed_a.squeeze(1), dim=-1)
        v_embed_a = F.normalize(v_embed_a.squeeze(1), dim=-1)
        ...
        l_embed = F.normalize(l_embed_u + l_embed_a, dim=-1)  
        l_embed_a = F.normalize(l_embed_a.squeeze(1), dim=-1)      
        ...
        with torch.no_grad():
            ...
            v_embed_m = F.normalize(v_embed_m_u + v_embed_m_a.squeeze(1), dim=-1)
            v_embed_m_all = torch.cat([v_embed_m.t(),self.image_queue.clone().detach()],dim=1)
            v_embed_m_a_all = torch.cat([F.normalize(v_embed_m_a.squeeze(1), dim=-1).t(),self.image_queue_a.clone().detach()],dim=1)                  
            ...
            l_embed_m = F.normalize(l_embed_m_u + l_embed_m_a.squeeze(1), dim=-1)
            l_embed_m_all = torch.cat([l_embed_m.t(),self.text_queue.clone().detach()],dim=1)
            l_embed_m_a_all = torch.cat([F.normalize(l_embed_m_a.squeeze(1), dim=-1).t(),self.text_queue_a.clone().detach()],dim=1)
        ...
        sim_i2t_a = v_embed_a @ l_embed_m_a_all / self.temp_a 
        sim_t2i_a = l_embed_a @ v_embed_m_a_all / self.temp_a 
                             
        loss_i2t = -torch.sum(F.log_softmax(sim_i2t, dim=1)*sim_targets,dim=1).mean()
        loss_t2i = -torch.sum(F.log_softmax(sim_t2i, dim=1)*sim_targets,dim=1).mean() 
        loss_i2t_a = -torch.sum(F.log_softmax(sim_i2t_a, dim=1)*sim_targets,dim=1).mean()
        loss_t2i_a = -torch.sum(F.log_softmax(sim_t2i_a, dim=1)*sim_targets,dim=1).mean() 

        loss_ita = (loss_i2t+loss_t2i)/2
        loss_ita_a = (loss_i2t_a + loss_t2i_a)/2
        
        loss = 0.4*loss_ita + 0.6*loss_ita_a
    
        idxs = concat_all_gather(idx)
        self._dequeue_and_enqueue(v_embed_m, l_embed_m, v_embed_m_a.squeeze(1), l_embed_m_a.squeeze(1), idxs)        
    ...
    @torch.no_grad()
    def _dequeue_and_enqueue(self, image_feat, text_feat, image_feat_a, text_feat_a, idxs):
        # gather keys before updating queue
        image_feats = concat_all_gather(image_feat)
        text_feats = concat_all_gather(text_feat)
        image_feats_a = concat_all_gather(image_feat_a)
        text_feats_a = concat_all_gather(text_feat_a)
        ...
        self.image_queue_a[:, ptr:ptr + batch_size] = image_feats_a.T
        self.text_queue_a[:, ptr:ptr + batch_size] = text_feats_a.T
