...
class BLIP_Retrieval(nn.Module):
    def __init__(self,                 
        ...
        self.temp_token = 0.07
        self.similarity_threshold = 0.2 
        
        self.negative_all_rank = negative_all_rank
        
        self.attention = SparseAttention(heads=8, attn_mode='all', local_attn_ctx=8, blocksize=8)
    ...
    def forward(self, image, caption, idx):
        ...
        v_patch_embed = self.attention(image_embeds[:,1:,:], image_embeds[:,0,:], image_embeds[:,0,:])
        l_token_embed = self.attention(text_output.last_hidden_state[:,1:,:], text_output.last_hidden_state[:,0,:], text_output.last_hidden_state[:,0,:])
        ...
        # # flattening as we need in-batch negatives?
        ...
        # mask = (1.0-mask_logits)

        # # multiply on inf is causing nans
        # loss_token_i2t = torch.sum(-mask*F.log_softmax(sim_token_i2t - mask_logits * torch.tensor(float('inf')), dim=-1)*sim_token_targets)/torch.sum(mask)
        # loss_token_t2i = torch.sum(-mask*F.log_softmax(sim_token_t2i - mask_logits * torch.tensor(float('inf')), dim=-1)*sim_token_targets)/torch.sum(mask)
        # loss_token_ita = 0.5*(loss_token_i2t + loss_token_t2i)
        
        # print(sim_token_i2t)
        # print(mask_logits.masked_fill(mask_logits.bool(), -torch.tensor(float('inf'))))
        # print((sim_token_i2t + mask_logits.masked_fill(mask_logits.bool(), -torch.tensor(float('inf')))))
        # print((sim_token_i2t - mask_logits * torch.tensor(float('inf'))==0).sum())


        # I dont think it is in-batch negatives for finegrained
