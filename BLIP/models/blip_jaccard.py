...
class BLIP_Retrieval(nn.Module):
    def __init__(self,                 
        ...
        self.query_threshold = 1.0/self.entries
        self.ans_threshold = 1.0/self.entries
        ...
        self.v_codebooks = nn.Embedding(self.entries, embed_dim)
        self.l_codebooks = nn.Embedding(self.entries, embed_dim)
    ...
    def forward(self, image, caption, idx):
        ...
        v_code_tokens = torch.arange(0, self.entries).unsqueeze(0).expand(len(v_embed), -1).to(image.device)
        v_code_embeddings = F.normalize(self.v_codebooks(v_code_tokens), dim=-1)
        l_code_tokens = torch.arange(0, self.entries).unsqueeze(0).expand(len(v_embed), -1).to(image.device)
        l_code_embeddings = F.normalize(self.l_codebooks(l_code_tokens), dim=-1)

        v_sim_q = torch.einsum('bd,bnd->bn', v_embed, v_code_embeddings)
        l_sim_a = torch.einsum('bd,bnd->bn', l_embed_m, l_code_embeddings)
        ...
        l_sim_q = torch.einsum('bd,bnd->bn', l_embed, l_code_embeddings)
        v_sim_a = torch.einsum('bd,bnd->bn', v_embed_m, v_code_embeddings)
    ...
    def init_random_codebook(self):
        ...
        for param in self.v_codebooks.parameters():
            param.requires_grad = False
        
        #nn.init.xavier_uniform_(self.l_codebooks.weight)  
        for param in self.l_codebooks.parameters():
            param.requires_grad = False
    ...
    def jaccard_loss(self, predictions, targets):
        ...
        loss = 1 - jaccard  # Minimizing the Jaccard index is equivalent to minimizing its complement
        return loss
