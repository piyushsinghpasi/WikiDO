...
class AttentionLayer(nn.Module):
    def __init__(self, cls_dim, token_dim):
        super(AttentionLayer, self).__init__()
        self.cls_linear = nn.Linear(cls_dim, token_dim)

    def forward(self, cls_representation, token_level_representations):
        # Apply linear transformation to CLS representation
        cls_linear = self.cls_linear(cls_representation)

        # Calculate attention scores
        attention_scores = F.softmax(torch.bmm(token_level_representations,cls_linear.unsqueeze(2)), dim=1)
        print(attention_scores.shape)

        # Compute new token representations
        new_token_representations = torch.bmm(token_level_representations.permute(1,0,2), attention_scores).permute(1,0,2)

        return new_token_representations
...
class BLIP_Retrieval(nn.Module):
    def __init__(self,                 
        ...
        self.attention_layer = nn.MultiheadAttention(embed_dim, 1, batch_first=True)
    ...
    def forward(self, image, caption, alpha, idx):        
        ...
        with torch.no_grad():
            ...
            # B x W x 256
            text_token_feat_m = F.normalize(self.text_proj_m(text_output_m.last_hidden_state[:,1:,:]),dim=-1) 
        ...
        image_patch_feat, image_patch_feat_weights = self.attention_layer(image_patch_feat, image_feat.unsqueeze(1), image_feat.unsqueeze(1))
        text_token_feat, text_token_feat_weights = self.attention_layer(text_token_feat, text_feat.unsqueeze(1), text_feat.unsqueeze(1))
