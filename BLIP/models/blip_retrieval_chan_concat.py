...
class BLIP_Retrieval(nn.Module):
    def __init__(self,                 
        ...
        self.vision_new_proj = nn.Linear(vision_width*2, embed_dim)
        self.text_new_proj = nn.Linear(text_width*2, embed_dim)
        ...
        self.vision_new_proj_m = nn.Linear(vision_width*2, embed_dim)
        self.text_new_proj_m = nn.Linear(text_width*2, embed_dim)
        
        self.model_pairs = [
                            [self.vision_new_proj,self.vision_new_proj_m],
                            [self.text_new_proj,self.text_new_proj_m]
                           ]       
        
        self.model_pairs_frozen = [
    ...
    def forward(self, image, caption, alpha, idx):        
        ...
        iip = torch.cat((image_embeds[:,1:,:], image_embeds[:,0,:].unsqueeze(1).expand(-1, 256, -1)), dim=2)
        #print(image_embeds[:,1:,:].shape)
        #print(image_embeds[:,0,:].unsqueeze(1).expand(-1, 256, -1).shape)
        #print(iip.shape)
        #exit()
        
        # Changed to: B x (P+1) x 256
        image_patch_feat = F.normalize(self.vision_new_proj(iip), dim=-1)
        ...
        # changed to: B x W(35) x 256
        ttp = torch.cat((text_output.last_hidden_state[:,1:,:], text_output.last_hidden_state[:,0,:].unsqueeze(1).expand(-1, 34, -1)), dim=2)
        text_token_feat = F.normalize(self.text_new_proj(ttp), dim=-1)
        ...
        with torch.no_grad():
            ...
            # B x P x 256
            image_patch_feat_m = F.normalize(self.vision_new_proj_m(torch.cat((image_embeds_m[:,1:,:], image_embeds_m[:,0,:].unsqueeze(1).expand(-1, 256, -1)), dim=2)), dim=-1)
            ...
            # B x W x 256
            text_token_feat_m = F.normalize(self.text_new_proj_m(torch.cat((text_output_m.last_hidden_state[:,1:,:], text_output_m.last_hidden_state[:,0,:].unsqueeze(1).expand(-1, 34, -1)), dim=2)), dim=-1)
