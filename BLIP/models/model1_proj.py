...
class BLIP_Retrieval(nn.Module):
    def __init__(self,                 
        ...
        self.agg_v_proj1 = nn.Linear(embed_dim, embed_dim)
        self.agg_l_proj1 = nn.Linear(embed_dim, embed_dim)

        self.dropout_v1 = nn.Dropout(p=0.2)
        self.dropout_v2 = nn.Dropout(p=0.2)
        ...
        self.agg_v_proj2 = nn.Linear(embed_dim, embed_dim)
        self.agg_l_proj2 = nn.Linear(embed_dim, embed_dim)
    ...
    def forward(self, image, caption, idx=None, inference=False):
        ...
        v_patch_embed = self.visual_encoder(image, return_layer=6) 
        ...
        print(len(text_output.hidden_states))
        print(text_output.hidden_states[0].size())
        exit()
        ...
        v_pooled = torch.mean(v_patch_embed, dim=1)
        l_pooled = torch.sum(l_pooled, dim=1)/l

        v_pooled_n = self.layernorm1(v_pooled)
        v_pooled_ = self.agg_v_proj2(self.dropout_v1(F.relu(self.agg_v_proj1(v_pooled_n))))
        v_pooled = v_pooled_n + self.dropout_v2(v_pooled_)

        l_pooled_n = self.layernorm2(l_pooled)
        l_pooled_ = self.agg_l_proj2(self.dropout_l1(F.relu(self.agg_l_proj1(l_pooled_n))))
        l_pooled = l_pooled_n + self.dropout_l2(l_pooled_)

        v_pooled = F.normalize(v_pooled, dim=-1)
        l_pooled = F.normalize(l_pooled, dim=-1)
