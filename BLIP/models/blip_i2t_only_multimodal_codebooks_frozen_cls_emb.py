...
class BLIP_Retrieval(nn.Module):
    ...
    def forward(self, image, caption, idx):
        ...
        text_word_emb = self.text_encoder(text.input_ids, attention_mask = text.attention_mask,                      
                                        return_dict = True, mode = 'text', return_word_embeddings=True)
    
        text_output = self.text_encoder(text.input_ids, attention_mask = text.attention_mask,                      
                                        return_dict = True, mode = 'text', return_word_embeddings=False)
        ...
        # B x T x D
        l_token_embed = self.text_proj(text_word_emb)
        ...
        text_token_domain, _ = self.text_domain_attention(l_token_embed,domain_rep,domain_rep)
        text_domain = text_token_domain.sum(dim=1) / text.attention_mask.sum(-1)


        v_embed = v_embed + text_domain
