...
def create_dataset(dataset, config, min_scale=0.5):
    ...
    if dataset=='pretrain':
        ...
        val_dataset = coco_karpathy_retrieval_eval(config['val_path'],config['test_path'], transform_test, config['image_root'], 'val') 
        test_dataset = coco_karpathy_retrieval_eval(config['val_path'],config['test_path'],transform_test, config['image_root'], 'test')          
    ...
    elif dataset=='retrieval_other':
        train_dataset = flickr30k_train(config['train_path'],transform_train, config['image_root'], config['ann_root'])
        val_dataset = flickr30k_retrieval_eval(config['val_path'],config['test_path'],transform_test, config['image_root'], config['ann_root'], 'val') 
        test_dataset = flickr30k_retrieval_eval(config['val_path'],config['test_path'],transform_test, config['image_root'], config['ann_root'], 'test')          
        return train_dataset, val_dataset, test_dataset 
        ...
        print("-"*100)
        print("Max words", config.get('max_words', 30))
        print("-"*100)
        train_dataset = WIDO_train_codebook(config['train_path'],transform_train, config['image_root'], max_words=config.get('max_words', 30))
        val_dataset = WIDO_eval_codebook(config['val_path'],config['test_path'],transform_test, config['image_root'], 'val', max_words=config.get('max_words', 30))
        test_dataset = WIDO_eval_codebook(config['val_path'],config['test_path'],transform_test, config['image_root'], 'test', max_words=config.get('max_words', 30))   
        return train_dataset, val_dataset, test_dataset 
