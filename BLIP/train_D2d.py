...
def evaluation(model, data_loader, device, config, args):
    ...
    for idx, (image, img_id, img_path, caption, topic) in enumerate(data_loader): 
        ...
        text_embed_dom,_ = model.domain_attention(text_embed.unsqueeze(1), domain_rep, domain_rep) 
        image = image.to(device) 
        image_feat = model.visual_encoder(image)   
        image_embed = model.vision_proj(image_feat[:,0,:])  
        image_embed_dom,_ = model.domain_attention(image_embed.unsqueeze(1), domain_rep, domain_rep)   
        text_embed = F.normalize(text_embed + text_embed_dom.squeeze(1), dim=-1)
        image_embed = F.normalize(image_embed + image_embed_dom.squeeze(1),dim=-1)      
    ...
    torch.save(text_embeds, "./D2d_itc_only_iid_text_embeds.pt")
    torch.save(image_embeds, "./D2d_itc_only_iid_image_embeds.pt")
    with open("./D2d_itc_only_iid_image_embeds.txt", 'w') as im_file:
        for item in img_paths:
            im_file.write(f"{item}\n")
        
    with open("./D2d_itc_only_iid_text_embeds.txt", 'w') as txt_file:
        for item in topics:
...
def main(args, config):
    ...
    from models.blip_D2d import blip_retrieval
