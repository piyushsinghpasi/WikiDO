...
from models.blip_ret_cls_token_attn import blip_retrieval
import utils
...
def evaluation(model, data_loader, device, config):
    ...
    for image, img_id in data_loader: 
        ...
        img_cls = F.normalize(image_embed[:,0,:], dim=-1)          
        image_embed = F.normalize(image_embed[:,1:,:],dim=-1)      
        image_feats.append(image_feat.cpu())
        image_cls.append(img_cls)
    ...
    for i in tqdm(range(len(image_embeds))):
        ...
        for j in range(len(text_embeds)):
            img_in,_ = model.attention_layer(image_embeds[i], image_cls[i].unsqueeze(1), image_cls[i].unsqueeze(1))
            txt_in,_ = model.attention_layer(text_embeds[j], text_feats[j].unsqueeze(1), text_feats[j].unsqueeze(1))
            sims_col = model.get_score(img_in, txt_in, cap_l[j], is_image_first=True)
    ...
    for i in tqdm(range(len(text_embeds))):
        ...
        for j in range(len(image_embeds)):
            img_in,_ = model.attention_layer(image_embeds[j], image_cls[j].unsqueeze(1), image_cls[j].unsqueeze(1))
            txt_in,_ = model.attention_layer(text_embeds[i], text_feats[i].unsqueeze(1), text_feats[i].unsqueeze(1))
            sims_col = model.get_score(txt_in, img_in, cap_l[i], is_image_first=False)
            sims_row.append(sims_col)
        sims_matrix.append(torch.cat(sims_row,dim=1))
    sims_matrix = torch.cat(sims_matrix, dim=0)
