...
from models.blip_retrieval_chan_concat import blip_retrieval
import utils
...
def evaluation(model, data_loader, device, config):
    ...
    for i in range(0, num_text, text_bs):
        ...
        text_embed = F.normalize(model.text_new_proj(torch.cat((text_output.last_hidden_state[:,1:,:], text_output.last_hidden_state[:,0,:].unsqueeze(1).expand(-1, 34, -1)), dim=2)), dim=-1)
    ...
    for image, img_id in data_loader: 
        ...
        image_embed = F.normalize(model.vision_new_proj(torch.cat((image_feat[:,1:,:], image_feat[:,0,:].unsqueeze(1).expand(-1, 256, -1)), dim=2)), dim=-1)
