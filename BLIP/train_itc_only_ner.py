...
def evaluation(model, data_loader, device, config, args):
    ...
    # torch.save(text_embeds, "./ood_text_embeds.pt")
    # torch.save(image_embeds, "./ood_image_embeds.pt")
    # with open("./ood_images.txt", 'w') as im_file:
    #     for item in img_paths:
    #         im_file.write(f"{item}\n")
        
    # with open("./ood_texts.txt", 'w') as txt_file:
    #     for item in topics:
...
def main(args, config):
    ...
    model = blip_retrieval(pretrained=config['pretrained'], image_size=config['image_size'], vit=config['vit'], 
                             ...
                             max_words=config.get('max_words', 35))
