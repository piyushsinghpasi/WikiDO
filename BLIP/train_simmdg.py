...
def evaluation(model, data_loader, device, config, args):
    ...
    for idx, (image, img_id, img_path, caption, topic) in enumerate(data_loader): 
        ...
        text_embed = F.normalize(model.text_proj_1(text_embed[:,:int(text_embed.shape[1]/2)]), dim = -1)
        # text_embed = F.tanh(text_embed)
        # text_embed = F.normalize(model.text_proj_1(text_embed),dim = -1)
        # text_embed = F.relu(text_embed)
        # text_embed = model.text_proj_2(text_embed)
        # text_embed = F.relu(text_embed)
        # text_embed = model.text_proj_3(text_embed)
        # text_embed = F.tanh(text_embed)
        ...
        image_embed = F.normalize(model.vision_proj_1(image_embed[:,:int(image_embed.shape[1]/2)]), dim = -1)
          


        # image_embed = F.tanh(image_embed)
        # image_embed = F.normalize(model.vision_proj_1(image_embed),dim = -1) 
        # image_embed = F.relu(image_embed)
        # image_embed = model.vision_proj_2(image_embed)
        # image_embed = F.relu(image_embed)  
        # image_embed = model.vision_proj_3(image_embed)  
...
def main(args, config):
    ...
    from models.blip_simmdg2 import blip_retrieval
