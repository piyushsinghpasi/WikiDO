...
def train(model, data_loader, optimizer, epoch, device, config, args):
    ...
    metric_logger.add_meter('loss_sense_ita', utils.SmoothedValue(window_size=1, fmt='{value:.4f}'))
    ...
    for i,metadata in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        loss_ita, loss_sense_ita = model(image, caption, idx=idx)         
        # loss = 0.5*loss_ita + 0.5*loss_sense_ita
        loss = loss_ita + loss_sense_ita
        ...
        metric_logger.update(loss_sense_ita=loss_sense_ita.item())
        ...
        wandb_stats = {
            ...
            "sense_loss": loss_sense_ita.item(),
...
def evaluation(model, data_loader, device, config, args):
    ...
    for idx, (image, img_id, img_path, caption, topic) in enumerate(data_loader): 
        ...
        B,D = image_embed.shape
        v_sense_embed = model.sense_proj(image_embed).view(B,model.num_senses,D)
        l_sense_embed = model.sense_proj(text_embed).view(B,model.num_senses,D)
        image_embed = F.normalize(v_sense_embed.mean(dim=1),dim=-1)    
        text_embed = F.normalize(l_sense_embed.mean(dim=1),dim=-1)     
        #image_embed = F.normalize(v_sense_embed,dim=-1)  
        #text_embed = F.normalize(l_sense_embed,dim=-1)     
...
def main(args, config):
    ...
    # from models.blip_finegrained import blip_retrieval
    from models.blip_baseline_sense_equi import blip_retrieval
    # from models.blip_baseline_sense_equi_sep import blip_retrieval
