...
def train(model, data_loader, optimizer, epoch, device, config, args, model_bma=None, alpha_k_cum=None):
    ...
    for i,metadata in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        elif args.method in ['correct_attn','self_modal_codebooks','sparse_codebooks','baseline_modified_loss',"domain_codebooks","separate_domain_codebooks","multimodal_codebooks","multimodal_codebooks_frozen","simple_codebooks", "vq_simple_codebooks", "simple_codebooks_modified_loss"]:
            (image, caption, idx, topic) = metadata
            image = image.to(device, non_blocking=True)
        else:
        ...
        elif args.method in ['correct_attn','self_modal_codebooks',"sparse_codebooks", "domain_codebooks","separate_domain_codebooks","multimodal_codebooks","multimodal_codebooks_frozen","simple_codebooks","simple_codebooks_modified_loss"]:
...
def evaluation(model, data_loader, device, config, args):
    ...
    for idx, (image, img_id, img_path, caption, topic) in enumerate(data_loader): 
        ...
        if args.method in ['vq','vq_itm_mom']:
            ...
            text_embed = model.text_norm(text_feat_domain.squeeze(1) + text_embed)
            image_embed = model.image_norm(image_feat_domain.squeeze(1) + image_embed)
        elif args.method == 'correct_attn':
            input_domain_tokens = torch.arange(0, model.entries).unsqueeze(0).expand(len(topic), -1).to(image.device)
            domain_rep = model.codebooks(input_domain_tokens) 
            image_feat_domain, image_attn_wts = model.image_domain_attention(image_embed.unsqueeze(1),domain_rep,domain_rep)
            text_feat_domain, text_attn_wts = model.text_domain_attention(text_embed.unsqueeze(1),domain_rep,domain_rep)
            image_feat_domain = image_feat_domain.squeeze(1)
            text_feat_domain = text_feat_domain.squeeze(1)
        elif args.method == "vq_simple_codebooks":
    ...
    text_embeds = torch.concat(text_embeds,dim=0)
    ...
    if args.method == 'correct_attn':
        # sims_matrix = torch.rand(len(image_embeds),len(text_embeds))
        # for im in tqdm(range(len(image_embeds))):
        #     imn = []
        #     for im_d in range(len(text_feats_domain)):
        #         imn.append(F.normalize(model.image_norm(image_embeds[im] + text_feats_domain[im_d]),dim=-1))
        #     imn = torch.stack(imn, dim=0)
        #     for tx in tqdm(range(len(text_embeds))):
        #         txn = []
        #         for tx_d in range(len(image_feats_domain)):
        #             txn.append(F.normalize(model.text_norm(text_embeds[tx] + image_feats_domain[tx_d]), dim=-1))
        #         txn = torch.stack(txn, dim=0)
        #         sim = imn @ txn.t()
        #         sims_matrix[im,tx] = torch.max(torch.max(sim))
                
        sims_matrix = torch.zeros(len(image_embeds), len(text_embeds), device=image_embeds.device)

        for im in tqdm(range(len(image_embeds))):
            imn = F.normalize(model.image_norm(image_embeds[im] + text_feats_domain), dim=-1)
            for tx in range(len(text_embeds)):
                txn = F.normalize(model.text_norm(text_embeds[tx] + image_feats_domain), dim=-1)
                sim = imn @ txn.t()
                sims_matrix[im, tx] = torch.max(torch.max(sim))

        # Extracting the maximum across both dimensions (im, tx)
        #sims_matrix = torch.max(sims_matrix, dim=2).values
        
        # batch_size_im = 16  # Set an appropriate batch size for the outer loop

        # # Assuming that image_embeds, text_embeds, text_feats_domain, image_feats_domain are already on GPU if available
        # sims_matrix = torch.zeros(len(image_embeds), len(text_embeds), device=image_embeds.device)

        # for start_im in tqdm(range(0, len(image_embeds), batch_size_im)):
        #     end_im = min(start_im + batch_size_im, len(image_embeds))
            
        #     batch_image_embeds = image_embeds[start_im:end_im]
        #     batch_text_feats_domain = text_feats_domain.unsqueeze(0).expand(len(batch_image_embeds), -1, -1)
            
        #     imn = F.normalize(model.image_norm(batch_image_embeds + batch_text_feats_domain), dim=-1)
            
        #     for tx in tqdm(range(len(text_embeds))):
        #         txn = F.normalize(model.text_norm(text_embeds[tx] + image_feats_domain), dim=-1)
        #         sim = imn @ txn.t()
        #         sims_matrix[start_im:end_im, tx] = torch.max(torch.max(sim, dim=2).values, dim=1).values
...
def main(args, config):
    ...
    if args.method == 'baseline':
        ...
        from models.blip_retrieval_correct_attn import blip_retrieval
    ...
    for epoch in range(0, config['max_epoch']):    
        ...
        if utils.is_main_process():  
            ...
            torch.save(save_obj, os.path.join(args.output_dir, 'checkpoint_best_{}.pth'.format(str(epoch))))
