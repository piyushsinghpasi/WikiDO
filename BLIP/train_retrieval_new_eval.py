...
def train(model, data_loader, optimizer, epoch, device, config, args, model_bma=None, alpha_k_cum=None):
    ...
    for i,metadata in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        elif args.method in ['baseline', 'baseline_modified_loss']:
...
def evaluation(model, data_loader, device, config, args):
    ...
    for idx, (image, img_id, img_path, caption, topic) in enumerate(data_loader): 
        ...
        if args.method in ['vq','vq_itm_mom']:
            ...
            image_feats_domain.append(image_feat_domain)
            text_feats_domain.append(text_feat_domain)
    ...
    if args.method not in ["sparse_codebooks","simple_codebooks"]:
    ...
    if args.method in ["sparse_codebooks", "simple_codebooks"]:
        # All x 1 x D
        image_feats_domain = torch.cat(image_feats_domain, dim=0)
        text_feats_domain = torch.cat(text_feats_domain, dim=0)

        # cross modal
        # T x I x D
        image_aware_text_rep = text_embeds.unsqueeze(1) + image_feats_domain.squeeze(1).unsqueeze(0)
        print("image_aware_text_rep", image_aware_text_rep.size())
        # I x T x D
        text_aware_image_rep = image_embeds.unsqueeze(1) + text_feats_domain.squeeze(1).unsqueeze(0)
        print("text_aware_image_rep", text_aware_image_rep.size())

        # T x I x D
        image_aware_text_rep = model.text_norm(image_aware_text_rep)
        image_aware_text_rep = F.normalize(image_aware_text_rep, dim=-1, p=2)
        # I x T x D
        text_aware_image_rep = model.image_norm(text_aware_image_rep)
        text_aware_image_rep = F.normalize(text_aware_image_rep, dim=-1, p=2)

        # text1 -> (t1, i1), (t1, i2), (t1, i3)
        # (i1, t1) (i1, t2) (i1, t3) ...
        # (i2, t1) (i2, t2) (i2, t3) ...
        # for each text1 rep: only (i1, t1) (i2, t1) (i3, t1) are valid representations

        # we have to pick top K
        # from (t1, i1) [(i1, t1) (i2, t1) (i3, t1)...]
        # from (t1, i2) [(i1, t1) (i2, t1) (i3, t1)...]
        # from (t1, i3) [(i1, t1) (i2, t1) (i3, t1)...]
        # ...

        # text : T x I x D  [(t1, i1) (t1, i2) (t1, i3)...]
        # image: T x I x D  [(i1, t1) (i2, t1) (i3, t1)...]
        # (t1, i1) [(i1, t1) (i2, t1) (i3, t1)...]
        # (t1, i2) [(i1, t1) (i2, t1) (i3, t1)...]
        # (t1, i3) [(i1, t1) (i2, t1) (i3, t1)...]

        # we have to identify top K image embeds 
        # (t1, i2) [(i1, t1) (i2, t1) (i3, t1)...]
        # as ITC is only filtering, ITM gets image_embeds and not cross-model attended feat

        # Below matrix multiplication is TOO EXPENSIVE 
        # So Doing this on the fly in topk for loop below

        # permute : I x T x D -> T x D x I 
        # T x I x I
        # sim_matrix_t2i = image_aware_text_rep @ text_aware_image_rep.permute(1, 2, 0)
        # small_batch_size = 256
        # sim_matrix_t2i = batched_matrix_multiply(
        #     matrix1=image_aware_text_rep, 
        #     matrix2=text_aware_image_rep.permute(1,2,0), 
        #     batch_size=small_batch_size
        # )

        
        # T, I, I = sim_matrix_t2i.size()
        # # T x I*I
        # sim_matrix_t2i = sim_matrix_t2i.reshape(T, I*I)
        # # T x I*I
        # image_idxs = torch.arange(I).unsqueeze(0).repeat(T, I)

        # # I x T x T
        # # sim_matrix_i2t = text_aware_image_rep @ image_aware_text_rep.permute(1, 2, 0)
        # sim_matrix_i2t = batched_matrix_multiply(
        #     matrix1=text_aware_image_rep,
        #     matrix2=image_aware_text_rep.permute(1, 2, 0),
        #     batch_size=small_batch_size
        # )
        # I, T, T = sim_matrix_i2t.size()

        # # I x T*T
        # sim_matrix_t2i = sim_matrix_t2i.reshape(I, T*T)
        # # I x T*T
        # text_idxs = torch.arange(T).unsqueeze(0).repeat(I, T)


    image_aware_text_rep_permuted = image_aware_text_rep.permute(1, 2, 0)
    text_aware_image_rep_permuted = text_aware_image_rep.permute(1, 2, 0)

    # not used in case of codebooks (but we need for len and stuff)
    ...
    for i,sims in tqdm(enumerate(metric_logger.log_every(sims_matrix[start:end], 1000, header))):
        if args.method in ["sparse_codebooks", "simple_codebooks"]:
            # I(1) x T x D @ I(1) x D x T
            # T x T
            # I 2 T
            # ith image : All text rep (T x D)
            # All text for ith image: All image rep (I x D)
            # Text: T x [i] x D
            # sims = T x T
            sims = text_aware_image_rep[i, :, :] @ image_aware_text_rep[:, i, :].t()
            # sims = text_aware_image_rep[i] @ image_aware_text_rep_permuted[i] 

            T, _ = sims.size()
            sims = sims.reshape(T*T)
            text_idxs = torch.arange(T).repeat(T).to(sims.device)
            #           t1 + i1 , t2 + i1
            # i1 + t1               Max
            # i1 + t2

        # sims is row-wise shape : T or T*T (for codebooks)
        # hence
    ...
    for i,sims in tqdm(enumerate(metric_logger.log_every(sims_matrix[start:end], 1000, header))): 
        ...
        if args.method in ["sparse_codebooks", "simple_codebooks"]:
            # T(1) x I x D @ T(1) x D x I
            # I x I
            # sims = image_aware_text_rep[i] @ text_aware_image_rep_permuted[i] 
            sims = image_aware_text_rep[i, :, :] @ text_aware_image_rep[:, i, :].t()

            I, _ = sims.size()
            sims = sims.reshape(I*I)
            image_idxs = torch.arange(I).repeat(I).to(sims.device)
...
def main(args, config):
    ...
    if args.method == 'baseline':
        ...
        from models.blip_retrieval_mmc_neg import blip_retrieval
    elif args.method == 'self_modal_codebooks':
